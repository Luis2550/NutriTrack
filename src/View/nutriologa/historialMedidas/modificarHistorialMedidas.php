...
<main class="main main_historialMed"> 
    ...
    <form id="nuevo" name="nuevo" method="POST" action="index.php?c=historialMedidas&a=actualizarHistorialMedidas" autocomplete="off">
        ...
        <label for="id_historial_clinico"></label>
        <input type="hidden" id="id_historial_clinico" name="id_historial_clinico" required value="<?php echo $data["historial_medidas"]["id_historial_clinico"]?>">
        
        <!-- Agregamos las validaciones directamente en los campos -->
        <label for="peso">Peso (KG)*</label>
        <input type="number" value="<?php echo $data['historial_medidas']['peso'] ?>" id="peso" min="5" max="500" name="peso" required oninput="validarPeso()">
        <div id="pesoError" style="color: red;"></div>

        <label for="estatura">Estatura (CM)*</label>
        <input type="number" value="<?php echo $data['historial_medidas']['estatura'] ?>" id="estatura" min="50" max="300" name="estatura" required min="50" max="300" oninput="validarTalla()">
        <div id="estaturaError" style="color: red;"></div>

        <label for="presion_s">Presión Arterial Sistólica:</label>
        <input type="number" value="<?php echo $data['historial_medidas']['presion_arterial_sistolica'] ?>"  id="presion_s" min="70" max="200" name="presion_s" required oninput="validarPresiones()">
        <div id="presion_sError" style="color: red;"></div>

        <label for="presion_d">Presión Arterial Diastólica:</label>
<input type="number" id="presion_d" value="<?php echo $data['historial_medidas']['presion_arterial_diastolica'] ?>" min="40" max="120" name="presion_d" required oninput="validarPresiones()">
<div id="presion_dError" style="color: red;"></div>

        <label for="fecha">Fecha:</label>
        <input type="date" id="fecha" name="fecha"  readonly value="<?php echo $data["historial_medidas"]["fecha"]?>">
